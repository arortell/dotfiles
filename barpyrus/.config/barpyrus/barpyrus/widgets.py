...
class Widget:
    ...
    def next_timeout(self):
        ...
        for w in self.subwidgets:
            ...
            if to is None:
                continue
            next_to = min(next_to, to) if next_to is not None else to
    ...
    def print_widget_tree(self, indent="", file=sys.stderr):
        print(
            '{}- Widget "{}" has {} subwidgets'.format(
                indent, type(self).__name__, len(self.subwidgets)
            ),
            file=file,
        )
...
class Switcher(Widget):
    ...
    def render(self, painter):
        ...
        painter.set_flag(p.overline | painter.underline, True)
        painter.space(3)
        for i, btn in enumerate(self.option_buttons):
            if i == self.selection:
